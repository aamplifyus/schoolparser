...
from eztrack.base.utils.errors import (
    EZTrackRuntimeError,
    EZTrackTypeError,
    EZTrackLookupError,
    EZTrackValueError,
...
from eztrack.base.utils.bids_utils.bidsio import BidsWriter, BidsLoader
...
class BidsRun(BaseBids):
    ...
    def __init__(self, bids_root, bids_fname: str, verbose: bool = False):
        ...
        logger.info(f"{self.bids_basename} is the file BIDS_BASENAME.")
    ...
    def append_channel_info(
        ...
        """
        Add a new column to the ch_names tsv file.
    ...
    def modify_channel_info(self, column_id: str, channel_id: str, value=None):
        ...
        if column_id not in channel_df.columns:
            raise EZTrackValueError(
                f"Column id {column_id} not in the ch_names tsv file. Please pass a correct column or "
                f"add to the tsv file"
            )
        if channel_id not in channel_df["name"].tolist():
            raise EZTrackLookupError(
    ...
    def delete_channel_info(self, column_id: str):
        """
        Remove column from the ch_names.tsv file.
    ...
    def _update_sidecar_json_channelcounts(self):
        ...
        Reads the current tsv file for the run's ch_names and updates the sidecar json with the channel type.
